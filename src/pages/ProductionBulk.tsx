...
export default function ProductionBulk() {
  const { projects: allProjects, loading } = useProjects()
  const { clients, loading: loadingClients } = useClients()
  const { factories, loading: loadingFactories } = useFactories()
  const bulkProjects = allProjects.filter(p => p.type === 'BULK')
  ...
  return (
    <Layout>
      <div className="container mx-auto py-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold tracking-tight">Production Bulk</h1>
          <p className="text-muted-foreground mt-1">
            Suivi de production des grosses commandes
          </p>
        </div>
